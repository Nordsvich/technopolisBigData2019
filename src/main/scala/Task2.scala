...
object Task2 {
  def main(args: Array[String]) {
    ...
    //load train df
    val df = sparkSession.read.option("header", "true").option("inferSchema", "true").csv("hw2/train.csv")
    df.printSchema()
    ...
    val testDf = sparkSession.read.option("header", "true").option("inferSchema", "true").csv("hw2/test.csv")
  ...
  def generateOutputFile(testDF: DataFrame, model: Model[_]): Unit = {
    ...
    castedDf.write.format("csv").option("header", "true").mode(SaveMode.Overwrite).save("hw2/output/")
  }
  ...
  def handleCategorical(column: String): Array[PipelineStage] = {
    ...
    val oneHot = new OneHotEncoderEstimator()
      .setInputCols(Array(s"${column}_index"))
      .setOutputCols(Array(s"${column}_onehot"))

    Array(stringIndexer, oneHot)
  }


  def accuracyScore(df: DataFrame, label: String, predictCol: String): Double = {
